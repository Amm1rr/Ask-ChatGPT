...
Private Sub Base_Resize (Width As Double, Height As Double)
	...
	For i = 0 To xpnl_TabBackground.NumberOfViews -1
		...
		Dim xlbl_Badge As B4XView = xpnl_Tab.GetView(3)
		...
		Dim BadgeWidth As Float = IIf(m_BadgeWithoutText = True,xBadgeProperties.Height, MeasureTextWidth(xlbl_Badge.Text,xlbl_Badge.Font) + xBadgeProperties.TextPadding)
		If BadgeWidth < xBadgeProperties.Height Then BadgeWidth = xBadgeProperties.Height
		
		If HaveIcon = True Then
			xlbl_Badge.SetLayoutAnimated(0,xiv_TabIcon.Left + xiv_TabIcon.Width + 5dip + xBadgeProperties.LeftPadding,xiv_TabIcon.Top - 5dip,BadgeWidth,BadgeWidth)
		Else
			xlbl_Badge.SetLayoutAnimated(0,xpnl_Tab.Width/2 + MeasureTextWidth(xTab.Text,xTabProperties.TextFont)/2 + xBadgeProperties.LeftPadding,xpnl_Tab.Height/2 - BadgeWidth,BadgeWidth,BadgeWidth)
		End If
		
		xlbl_Badge.SetColorAndBorder(xBadgeProperties.BackgroundColor,0,0,BadgeWidth/2)
...
Private Sub CreateTab(xTab As ASTabMenuAdvanced_Tab,xTabProperties As ASTabMenuAdvanced_TabProperties)
	...
	xpnl_Tab.AddView(xlbl_Badge,0,0,0,0)
